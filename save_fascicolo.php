...
function write_csv_assoc($filePath, $headers, $rows, $delimiter = ';') {
    $backupDir = __DIR__ . DIRECTORY_SEPARATOR . 'backup_fascicoli';
    if (!is_dir($backupDir)) {
        @mkdir($backupDir, 0775, true);
    }

    if (file_exists($filePath)) {
        $backupPath = $backupDir . DIRECTORY_SEPARATOR . 'fascicoli_' . date('Ymd_His') . '.csv';
        @copy($filePath, $backupPath);
    }

    $tmpPath = __DIR__ . DIRECTORY_SEPARATOR . 'fascicoli_tmp_' . uniqid('', true) . '.csv';
...
if (!$result) {
    redirect_back(false, 'Impossibile leggere fascicoli.csv', $returnQuery);
}

[$headers, $rows] = $result;

$requiredHeaders = ['FASCICOLO','SOTTOFASCICOLO','CATEGORIA','CLASSE','IND_CATEGORIA','IND_CLASSE','TIPO'];
...
$newRow = [
    'FASCICOLO' => trim((string)($_POST['FASCICOLO'] ?? '')),
    'SOTTOFASCICOLO' => trim((string)($_POST['SOTTOFASCICOLO'] ?? '')),
    'CATEGORIA' => trim((string)($_POST['CATEGORIA'] ?? '')),
    'CLASSE' => trim((string)($_POST['CLASSE'] ?? '')),
    'IND_CATEGORIA' => trim((string)($_POST['IND_CATEGORIA'] ?? '')),
    'IND_CLASSE' => trim((string)($_POST['IND_CLASSE'] ?? '')),
    'TIPO' => trim((string)($_POST['TIPO'] ?? ''))
];

if ($action === 'insert') {
    if ($newRow['FASCICOLO'] === '') {
        redirect_back(false, 'Il campo FASCICOLO è obbligatorio', $returnQuery);
    }

    array_unshift($rows, $newRow);

    if (!write_csv_assoc($filePath, $headers, $rows, ';')) {
        redirect_back(false, 'Impossibile salvare fascicoli.csv', $returnQuery);
    }
